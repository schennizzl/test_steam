select
    json_format(
        cast(
            map(
                array['appid', 'game_name', 'current_players', 'source_file', 'ingested_at', 'dt', 'hour'],
                array[
                    cast(cast(appid as bigint) as json),
                    cast(cast(game_name as varchar) as json),
                    cast(cast(current_players as integer) as json),
...
from {{ source('landing', 'game_online_files') }}
