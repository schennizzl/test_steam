...
FETCH_SCRIPT = f"{PROJECT_DIR}/fetch_twitch_channels.py"
GAMES_FILE = f"{PROJECT_DIR}/games.txt"
DBT_PROJECT_DIR = f"{PROJECT_DIR}/dbt_steam"
S3_BUCKET = "raw"
S3_PREFIX = "steam/landing/twitch_channels/dt={{ ds }}/hour={{ execution_date.strftime('%H') }}"
FILE_NAME = "twitch_channels_{{ ts_nodash }}.ndjson"
...
def sync_landing_twitch_channels_partitions() -> None:
    conn = _trino_connect()
    cur = conn.cursor()
    cur.execute("CALL system.sync_partition_metadata('landing', 'twitch_channels_files', 'ADD')")
    cur.fetchall()
    conn.close()
...
with DAG(
    ...
    load_raw_trino = BashOperator(
        ...
        bash_command=(
            ...
            "dbt run --profiles-dir . --select raw_steam_twitch_channels stg_steam_twitch_channels"
        ),
    )
    ...
    test_dbt_models = BashOperator(
        ...
        bash_command=(
            ...
            "--select raw_steam_twitch_channels stg_steam_twitch_channels"
        ),
    )

    ensure_dirs >> fetch_twitch_channels >> upload_minio >> sync_landing_partitions >> load_raw_trino >> test_dbt_models
