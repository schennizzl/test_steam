...
FETCH_SCRIPT = f"{PROJECT_DIR}/fetch_twitch_viewers.py"
GAMES_FILE = f"{PROJECT_DIR}/games.txt"
DBT_PROJECT_DIR = f"{PROJECT_DIR}/dbt_steam"
S3_BUCKET = "raw"
S3_PREFIX = "steam/landing/twitch_viewers/dt={{ ds }}/hour={{ execution_date.strftime('%H') }}"
...
def sync_landing_twitch_viewers_partitions() -> None:
    conn = _trino_connect()
    cur = conn.cursor()
    cur.execute("CALL system.sync_partition_metadata('landing', 'twitch_viewers_files', 'ADD')")
    cur.fetchall()
    conn.close()
...
with DAG(
    ...
    load_raw_trino = BashOperator(
        ...
        bash_command=(
            ...
            "dbt run --profiles-dir . --select raw_steam_twitch_viewers stg_steam_twitch_viewers"
        ),
    )
    ...
    test_dbt_models = BashOperator(
        ...
        bash_command=(
            ...
            "--select raw_steam_twitch_viewers stg_steam_twitch_viewers"
        ),
    )

    ensure_dirs >> fetch_twitch_viewers >> upload_minio >> sync_landing_partitions >> load_raw_trino >> test_dbt_models
