...
from fetch_twitch_viewers import (
    STREAMS_URL,
    build_api_headers,
...
def main() -> int:
    ...
    for appid, game_name, twitch_lookup_name, configured_twitch_category_id in load_games(args.games_file):
        category = None
        if configured_twitch_category_id is not None:
            category = get_category_by_id(category_id=configured_twitch_category_id, timeout=args.timeout, headers=headers)
        if category is None:
